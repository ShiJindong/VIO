...
void Problem::AddOrderingSLAM(std::shared_ptr<myslam::backend::Vertex> v) {
    if (IsPoseVertex(v)) {
        v->SetOrderingId(ordering_poses_);        // ordering id是在problem中排序后的id，用于寻找雅可比对应块
...
bool Problem::Solve(int iterations, int &used_iterations, double &used_time) {
...
//                stop = (b_max <= 1e-12);
    ...
    used_iterations = iter;
    std::cout << "used iterations: " << iter << " steps" << std::endl;
    used_time = t_solve.toc();
...
void Problem::MakeHessian() {
    ...
    for (auto &edge: edges_) {
        ...
        auto verticies = edge.second->Verticies();    // 这里每条边连接3个顶点
        ...
        for (size_t i = 0; i < verticies.size(); ++i) {
            ...
            for (size_t j = i; j < verticies.size(); ++j) {
                ...
                /*
                // set corresponding matrix block of 0. and 1. frame in Hessian as infinity, such that 0. and 1. Frame are fixed.
                if(v_i->Id() == v_j->Id())
                    if(v_i->Id() < 2) {
                        for(ulong k = 0; k < dim_i; ++k)
                            H(index_i + k, index_i + k) += 1e5;
                    }
                */
            ...
            // 注意这样计算b的话，成员变量b_就是是带负号的: H * delta_x = b_
            b.segment(index_i, dim_i).noalias() -= JtW * edge.second->Residual();   // Jt: 维度6*1  W: 维度1*1  r: 维度1*1
        }
...
//    std::cout << svd.singularValues() <<std::endl;
    ...
    // 矩阵块操作: topLeftCorner, bottomLeftCorner, topRightCorner, bottomRightCorner(p,q)
    Hessian_.topLeftCorner(ordering_poses_, ordering_poses_) += H_prior_;
    // 向量块操作: head(n), tail(n), segment(i, n)
...
void Problem::SolveLinearSystem() {
    ...
    if (problemType_ == ProblemType::GENERIC_PROBLEM) {
        ...
        // PCG solver
        MatXX H = Hessian_;
        // 将原来的AddLambdatoHessianLM();移动到SolveLinearSystem()内部
        for (ulong i = 0; i < Hessian_.cols(); ++i) {
            H(i, i) += currentLambda_;
        }
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        delta_x_ = Hessian_.inverse() * b_;      // 注意这里b_是带负号的
...
void Problem::TestMarginalize() {
    ...
    // 将矩阵Amm中小于eps的特征值直接置为0，再反推出Amm，从而来简化后续矩阵运算
