...
void EdgeReprojection::ComputeResidual() {
    // 路标点位姿变换关系:   在相机位姿i下坐标 -> 在IMU位姿i下的坐标通过(Qi, Pi)转换到 ->  在世界系下的坐标 -> 通过(Qj, Pj)转换到 在IMU位姿j下的坐标 -> 在相机位姿j下坐标
    ...
    Vec3 pts_camera_i = pts_i_ / inv_dep_i;            // 路标点i在相机坐标系下非归一化的坐标
    Vec3 pts_imu_i = qic * pts_camera_i + tic;         // 路标点i在imu坐标系下的坐标
    Vec3 pts_w = Qi * pts_imu_i + Pi;                  // 所以 我们优化的Qi, Pi是IMU的位姿，等于世界坐标系下的位姿
    Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);      // pts_w = Qj * pts_imu_j + Pj
    Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic);    // pts_imu_j = qic * pts_camera_j + tic
...
//    reduce = information_ * reduce;
    ...
    ///------------- check jacobians -----------------
//    {
//        // 数值法验证雅可比: 我们在某一个变量(比如逆深度)上加上一个小量，然后利用(tmp_residual - residual_)/eps 计算数值雅可比
