...
void IMU::addIMUnoise(MotionData& data)
{
    ...
    // 带高斯白噪声和bias的陀螺仪 角加速度 信号 imu_gyro += sigma_g * w_g[k] / sqrt(delta_t) + b_g  (VIO课件第2讲式10，15，16, 26)
    data.imu_gyro = data.imu_gyro + gyro_sqrt_cov * noise_gyro / sqrt( param_.imu_timestep ) + gyro_bias_;

    Eigen::Vector3d noise_acc(noise(generator_),noise(generator_),noise(generator_));
    Eigen::Matrix3d acc_sqrt_cov = param_.acc_noise_sigma * Eigen::Matrix3d::Identity();
    // 带高斯白噪声和bias的加速度计 加速度 信号 imu_acc += sigma_a * w_a[k] / sqrt(delta_t) + b_a (VIO课件第2讲式27)
    data.imu_acc = data.imu_acc + acc_sqrt_cov * noise_acc / sqrt( param_.imu_timestep ) + acc_bias_;

    // gyro_bias update (bias每时每刻都在随机游走，所以每个时刻都要更新)
    Eigen::Vector3d noise_gyro_bias(noise(generator_),noise(generator_),noise(generator_));
    // 离散化的bias随机游走：bd[k] = bd[k-1] + sigma_b_g * sqrt(delta_t) * w_b_g[k] (VIO课件第2讲式20，21)
...
MotionData IMU::MotionModel(double t)
{
    ...
    Eigen::Vector3d position( ellipse_x * cos( K * t) + 5, ellipse_y * sin( K * t) + 5,  z * sin( K1 * K * t ) + 5);   // (5,5,5)作为运动中心点
...
//    Eigen::Vector3d eulerAnglesRates(0.,0. , K);      // euler angles 的导数
    ...
    Eigen::Vector3d imu_gyro = eulerRates2bodyRates(eulerAngles) * eulerAnglesRates;   //  euler rates trans to body gyro (VIO课件第2讲式37)
...
void IMU::testImu(std::string src, std::string dist)
{
	...
	MotionData imupose_last = imudata[i-1];
        ...
        // Eigen::Vector3d dtheta_half =  imupose.imu_gyro * dt /2.0;    // 欧拉积分
	Eigen::Vector3d dtheta_half =  (imupose_last.imu_gyro * dt +  imupose.imu_gyro * dt)/2.0/2.0;	 // 中值积分
	...
	*/
        ...
        /// 中值积分
	Qwb = Qwb_last * dq;
	Eigen::Vector3d acc_w = 0.5 * (Qwb_last * imupose_last.imu_acc + gw + Qwb * imupose.imu_acc + gw);  
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        Vw = Vw + acc_w * dt;
	Qwb_last = Qwb;
