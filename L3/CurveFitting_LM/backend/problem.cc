...
bool Problem::Solve(int iterations) {
    ...
    vector<double, std::allocator<double>> currentChi_vec;
    vector<double, std::allocator<double>> currentLambda_vec;
...
//                stop = (b_max <= 1e-12);
    ...
    // draw the curve
    draw_curve(currentChi_vec, "currentChi");
    draw_curve(currentLambda_vec, "currentLambda");
...
void Problem::draw_curve(const vector<double, std::allocator<double>> &currentValue_vec, string name){
    // 参考 Pangolin Tutorial: https://github.com/yuntianli91/pangolin_tutorial/blob/master/task5/main.cpp
...
bool Problem::IsGoodStepInLM() {
    ...
    if(update_strategy_ ==1)
    {
        ...
        MatXX diag_Hessian = MatXX::Zero(Hessian_size, Hessian_size);
    ...
    else if(update_strategy_ == 2)
    {
        ...
        alpha = std::max(alpha, 1e-1);      // make sure it's non-zero
